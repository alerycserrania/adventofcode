...
main = do
  ...
  let distances = (bfs [posE] Map.empty 0 graph)
  let distancesToA = filter ((=='a'). fst) . map (\((row, col), d) -> (input!!row!!col, d)) $ (Map.toList distances)
  print (minimum . map snd $ distancesToA)
...
toArc m = concat [getArc row col | row <- [0..(length m)-1], col <- [0..(length (m!!row))-1]]
  where 
    ...
    reachable (rowA, colA) (rowB, colB) =
      (if isAtLeastOneStepBelow (m!!rowA!!colA, m!!rowB!!colB) then [((rowA, colA), (rowB, colB))] else [])
      ++ (if isAtLeastOneStepBelow (m!!rowB!!colB, m!!rowA!!colA) then [((rowB, colB), (rowA, colA))] else [])
    isAtLeastOneStepBelow (c1, c2) = (ord (parseChar c1) - 1) <= ord (parseChar c2)
      where 
        parseChar 'S' = 'a'
        parseChar 'E' = 'z'
        parseChar c = c 
