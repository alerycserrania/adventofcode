...
main = do
  input <- getInput
  let packets = map (\p -> (parseList (p!!0), parseList (p!!1))) (splitOn [] input)
  let order = map (\(b,c) -> compRecList (head b)(head c)) packets
  let solution = filter ((==LT) . snd) $ zip [1..] order
  print (sum . map fst $ solution) 
